...
class MeteoFranceConnector:
    """
    Usage: meteo_data = MeteoFranceConnector(datetime.datetime.strptime('2025-03-01', '%Y-%m-%d')).to_pandas()
    """
    ...
    def __init__(self, from_dt : datetime.datetime = datetime.datetime(2024, 12, 6, 0, 0, 0), to_dt : datetime.datetime = None):
        if to_dt is None:
            to_dt = datetime.date.today()
        from_dt_str = from_dt.strftime('%Y-%m-%dT%H:00:00Z')
        to_dt_str = to_dt.strftime('%Y-%m-%dT%H:00:00Z')
        url = self.DATA_NODE.format(station = self.SELECTED_STATION, from_dt = from_dt_str, to_dt = to_dt_str)
        head = {
            'accept' : '*/*',
            'apikey' : self.APIKEY
        }
        print(f'Chargement de données pour la période: {from_dt_str} - {to_dt_str}')
        try:
