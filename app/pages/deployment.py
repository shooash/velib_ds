...
- **BIG API**: service principale de l'entrainement de modèles avec des librairies NVidia pour des flux optimisés;
...
OUTRO = """
Les données principales pour l'entrainement de modèle sont récupérées de l'API Vélib' dans le dataset historique sur le serveur PostgreSQL.
La base de données et le service de la récupération de données sont hébergé sur Google Cloud Platform.

Les données climatologique utilisées pour l'entrainement de modèle proviennent de l'API MétéoFrance.

Les prévisions météorologiques nécessaires pour les prédictions sont chargées depuis le service OpenWeatherMap.
""".strip()

def show():
    st.title("Deployment du service")
    st.markdown(INTRO)
