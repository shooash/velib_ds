...
def test_velibdata_extract_new():
    ...
    data.extract()
    assert len(data.velib_data) > 1400 * 31 * 24
    assert len(data.meteo_data) >= 31 * 24

def test_velibdata_use_cache():
    global data
    data = VelibData(cache=True, update_cache=False)
    data.extract()
    assert len(data.meteo_data) > 0
    assert len(data.velib_data) > 0
...
def test_transformer_split():
    ...
    assert len(df) == len(data.data)
    assert len(df_train) > 0
    assert len(df_test) > 0
    assert list(df.columns) == list(df_train.columns)
